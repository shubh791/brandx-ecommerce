...
if (!fs.existsSync('app/account')) {
  fs.mkdirSync('app/account', { recursive: true });
}
if (!fs.existsSync('app/profile')) {
  fs.mkdirSync('app/profile', { recursive: true });
}

const accountPageCss = `.pageWrapper {
  --acc-canvas: #f4f2ed;
  --acc-ink: #09090b;
  --acc-muted: #65656d;
  --acc-line: rgba(9, 9, 11, 0.14);
  --acc-accent: #f2cf45;
  min-height: 100vh;
  display: flex;
...
/* =========================================================
   2. MAIN ACCOUNT DASHBOARD LAYOUT
   ========================================================= */
.dashboardShell {
  width: min(100%, 82rem);
  margin-inline: auto;
...
.mobileTabsBar::-webkit-scrollbar {
  display: none;
}
...
fs.writeFileSync('app/account/AccountPage.module.css', accountPageCss, 'utf-8');

// Create app/account/page.js
const accountPageJsx = `'use client';
...
fs.writeFileSync('app/account/page.js', accountPageJsx, 'utf-8');

// Create app/profile/page.js
const profileRedirectJsx = `'use client';
...
fs.writeFileSync('app/profile/page.js', profileRedirectJsx, 'utf-8');
console.log('Account & Profile pages created successfully!');
