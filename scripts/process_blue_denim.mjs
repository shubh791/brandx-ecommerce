...
const files = fs.readdirSync(brainDir).filter(f => f.startsWith('blue_denim_jeans') && f.endsWith('.jpg'));
...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    ...
    // Background is near white (r, g, b > 230 and neutral)
...
const outputPath = 'public/images/hero/hero-blue-denim.png';
await sharp(out, { raw: { width, height, channels: 4 } })
  .trim()
  .png()
  .toFile(outputPath);

console.log('Saved transparent blue denim to:', outputPath);
