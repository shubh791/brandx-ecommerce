  ...
  useGSAP(() => {
    ...
    intro
      ...
      .from(\`.\${styles.fashionObject}\`, {
        opacity: 0,
        x: (index) => [-50, 75, 40, -70, 50, 95][index],
        y: (index) => [-60, 55, 40, 70, 85, 90][index],
        scale: 0.75,
        duration: 1.05,
        stagger: 0.07,
      }, 0.2)
...
/* 2. Washed Black/Grey Denim Jeans (Right side) */
.denim {
  top: 9%;
  right: -2%;
  z-index: 4;
...
/* 3. Small Blue Denim Jeans (Compact indigo accent) */
.blueDenim {
  top: 14%;
  right: 21%;
  z-index: 3;
...
.blueDenim .objectPlane {
  transform: translate3d(0,0,95px) rotateY(6deg) rotateZ(-6deg);
}
...
/* 6. Sneakers (Lower-center/right foreground) */
.sneakers {
  right: 6%;
  bottom: 5%;
  z-index: 8;
...
console.log('Successfully updated HeroBanner with both black/grey denim and small blue denim jeans!');
