...
const fashionObjects = [
  ...
  { id: 'blue-denim', src: '/images/hero/hero-blue-denim.png', alt: 'Premium indigo blue denim jeans', className: styles.blueDenim, depth: 0.8, width: 928, height: 1856 },
  ...
  useGSAP(() => {
    ...
    intro
      ...
      .from(\`.\${styles.fashionObject}\`, {
        opacity: 0,
        x: (index) => [-50, 30, 75, -70, 50, -35, 95][index],
      ...
      .from(\`.\${styles.visualCaption}\`, { opacity: 0, y: 12, duration: 0.55 }, '-=0.2');
    ...
    gsap.to(\`.\${styles.denim} .\${styles.objectPlane}\`, {
      y: 6, rotateZ: -1.0, duration: 4.4, repeat: -1, yoyo: true, ease: 'sine.inOut',
    });
    gsap.to(\`.\${styles.blueDenim} .\${styles.objectPlane}\`, {
      y: -5, rotateZ: 1.2, duration: 4.0, repeat: -1, yoyo: true, ease: 'sine.inOut',
    });
    ...
    <section ref={heroRef} className={styles.hero} aria-labelledby="brandx-hero-title">
      ...
      <div className={styles.shell}>
        ...
        <div ref={visualRef} className={styles.visual} aria-label="BRAND X floating fashion collection">
          ...
          {/* Editorial Caption Directly Under Artwork with Spacing */}
...
/* 2. Small Indigo Blue Denim (Spaced to left-center of right area) */
.blueDenim {
  top: 4%;
  left: 54%;
  z-index: 3;
...
.blueDenim .objectPlane {
  transform: translate3d(0,0,85px) rotateY(-5deg) rotateZ(-3deg);
}
...
/* 3. Washed Black/Grey Denim Jeans (Right side with breathing space) */
.denim {
  top: 8%;
  right: -3%;
  z-index: 4;
...
.denim .objectPlane {
  transform: translate3d(0,0,80px) rotateY(-8deg) rotateZ(9deg);
}
...
/* 6. NEW Black Streetwear Runner (Center gap between bag and white sneaker) */
.blackSneaker {
  bottom: 6%;
  left: 30%;
  z-index: 8;
...
console.log('HeroBanner.jsx and HeroBanner.module.css successfully updated with black sneaker, jeans spacing, and caption spacing!');
