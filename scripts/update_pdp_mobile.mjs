...
// 1. Update ProductDetailPage.module.css
let css = fs.readFileSync('app/product/[id]/ProductDetailPage.module.css', 'utf-8');

const updatedMobilePdpCss = `
/* Mobile Gallery & Sticky Bar Enhancements */
.mobileStickyBtnGroup {
  display: flex;
  align-items: center;
  gap: 0.5rem;
}

.mobileStickyWishlistBtn {
  width: 2.75rem;
  height: 2.75rem;
  display: grid;
  place-items: center;
...
if (!css.includes('mobileStickyWishlistBtn')) {
  css += updatedMobilePdpCss;
  fs.writeFileSync('app/product/[id]/ProductDetailPage.module.css', css, 'utf-8');
}

// 2. Update app/product/[id]/page.js
...
export default function ProductDetailPage({ params }) {
  ...
  // IntersectionObserver to only show sticky bottom bar when main CTA scrolls out of view
  useEffect(() => {
  ...
  return (
    <div className={styles.pageWrapper}>
        ...
        <section className={styles.productSection}>
          <div className={styles.productLayout}>
            ...
            <div className={styles.infoStickyCol}>
              ...
              {/* Quantity & Large Conversion CTAs (Observed by ref for sticky bar) */}
              <div ref={mainCtaRef} className={styles.ctaBlock}>
      ...
      {/* 5. Mobile Sticky Bottom CTA Bar (ONLY appears when main CTA is scrolled out of view) */}
      <AnimatePresence>
        {!isMainCtaVisible && (
          <motion.div
            initial={{ y: '100%' }}
          ...
          >
            ...
            <div className={styles.mobileStickyBtnGroup}>
              <button
                type="button"
                onClick={() => toggleWishlist(product)}
                className={[
                  styles.mobileStickyWishlistBtn,
                  wishlisted ? styles.wishlistActionBtnActive : '',
                ].join(' ')}
                aria-label={wishlisted ? 'Remove from Wishlist' : 'Add to Wishlist'}
              >
...
console.log('PDP mobile sticky bar logic and IntersectionObserver added successfully!');
